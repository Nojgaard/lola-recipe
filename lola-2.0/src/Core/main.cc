...
int main(int argc, char **argv)
{
    ...
    RT::rep->status("preprocessing net");
    Net::preprocess();
    ...
    delete symbolTables;
    RT::rep->status("finished preprocessing");
    ...
    if (RT::args.check_arg != check_arg_none)
    {
        //======================
        // (5) the actual check
        //======================

        const ternary_t result = task.getResult();
        ...
        {
            RT::rep->status("%llu markings, %llu edges", task.getMarkingCount(), task.getEdgeCount());
        }

        // add data to JSON if required
        for (unsigned int i = 0; i < RT::args.jsoninclude_given; ++i)
        {
            switch (RT::args.jsoninclude_arg[i])
            {
            case jsoninclude_arg_state:
            {
                RT::data["state"] = JSON();
                const capacity_t *current = task.getMarking();
                if (current)
