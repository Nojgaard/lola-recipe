...
\brief Implementation of the CTL AU operator.
*/
...
void AUFormula::initAtomics(NetState &ns)
{
    phi->initAtomics(ns);
    psi->initAtomics(ns);
}

void AUFormula::updateAtomics(NetState &ns, arrayindex_t t)
{
    phi->updateAtomics(ns, t);
    psi->updateAtomics(ns, t);
}

void AUFormula::revertAtomics(NetState &ns, arrayindex_t t)
{
...
void AUFormula::gatherPayloadInformation(arrayindex_t &numDFS,
...
void AUFormula::setPayloadInformation(arrayindex_t cachedResultOffset,
                                      size_t payloadSize)
{
                               ...
                               fairness_data &fairness)
{
    ...
    while (true)
    {
        if (currentFirelistIndex--)
        {
            ...
            if (fairness.weak_backlist[currentFirelist[currentFirelistIndex]] != (uint32_t) - 1)
            {
            ...
            if (fairness.strong_backlist[currentFirelist[currentFirelistIndex]] != (uint32_t) - 1)
            {
...
FormulaInfo *AUFormula::getInfo() const
{
