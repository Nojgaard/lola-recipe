...
ATrans *build_alternating(tl_Node *p);

/********************************************************************\
|*              Generation of the alternating automaton             *|
\********************************************************************/

int calculate_node_size(tl_Node *p) /* returns the number of temporal nodes */
{
    switch (p->ntyp)
    {
    case AND:
...
int calculate_sym_size(tl_Node *p) /* returns the number of predicates */
{
    switch (p->ntyp)
    {
    case AND:
...
int already_done(tl_Node *p) /* finds the id of the node, if already explored */
{
    int i;
    for (i = 1; i < node_id; i++)
...
ATrans *build_alternating(tl_Node *p) /* builds an alternating automaton for p */
{
