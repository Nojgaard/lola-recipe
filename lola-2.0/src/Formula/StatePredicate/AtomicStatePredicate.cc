...
void AtomicStatePredicate::setUpSet()
{
    ...
    arrayindex_t *up1 = reinterpret_cast<arrayindex_t *>(malloc(Net::Card[TR] * SIZEOF_ARRAYINDEX_T));
    arrayindex_t *up2 = reinterpret_cast<arrayindex_t *>(malloc(Net::Card[TR] * SIZEOF_ARRAYINDEX_T));
    cardUp = 0;

    // idea: for each p: merge up1 and post places of pos / pre places of neg into up2
    for (arrayindex_t i = 0; i < cardPos; i++)
    {
        const arrayindex_t p = posPlaces[i];
        const arrayindex_t cardP = Net::CardArcs[PL][POST][p];

        arrayindex_t a = 0; // index in up1
        arrayindex_t b = 0; // index in arc list of place
        arrayindex_t c = 0; // index in up2 (result)
    ...
    for (arrayindex_t i = 0; i < cardNeg; i++)
    {
        const arrayindex_t p = negPlaces[i];
        const arrayindex_t cardP = Net::CardArcs[PL][PRE][p];
    ...
    up = reinterpret_cast<arrayindex_t *>(realloc(up1, cardUp * SIZEOF_ARRAYINDEX_T));
    free(up2);
}
