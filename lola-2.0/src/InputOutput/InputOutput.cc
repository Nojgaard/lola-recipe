                         ...
                         int, int last_column) const
{
    ...
    for (int i = 0; i < static_cast<int>(strlen(line)); ++i)
    {
        ...
        if (i >= first_column - 1 && i <= last_column - 1)
        {
            std::stringstream ss;
            ss << line[i];
            fprintf(stderr, "%s", RT::rep->markup(MARKUP_BAD, ss.str().c_str()).str());
        }
        else
        {
    ...
    for (int i = 0; i < static_cast<int>(strlen(line)); ++i)
    {
        ...
        if (i == first_column - 1)
        {
            fprintf(stderr, "%s", RT::rep->markup(MARKUP_GOOD, "^").str());
        }
        else
        {
