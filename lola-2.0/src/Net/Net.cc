...
void Net::swapPlaces(arrayindex_t left, arrayindex_t right)
{
    ...
    /*  arrayindex_t tempcarddisabled = Place::CardDisabled[left];
        Place::CardDisabled[left] = Place::CardDisabled[right];
        Place::CardDisabled[right] = tempcarddisabled;

        arrayindex_t* tempdisabled = Place::Disabled[left];
...
void Net::preprocess()
{
    ...
    *******************************/
    ...
    Place::CardDisabled = new arrayindex_t[cardPL]();
    Place::Disabled = new arrayindex_t *[cardPL];
    for (arrayindex_t p = 0; p < cardPL; p++)
    {
        // initially: no disabled transistions (through CardDisabled = 0)
        // correct values will be achieved by initial checkEnabled...
        Place::Disabled[p] = new arrayindex_t[Net::CardArcs[PL][POST][p]];
    }
    Transition::PositionScapegoat = new arrayindex_t[cardTR];
