...
NetState *NetState::createNetStateFromInitial()
{
    ...
    ns->PositionScapegoat = new arrayindex_t[Net::Card[TR]];
    for (arrayindex_t i = 0; i < Net::Card[TR]; i++)
    {
    ...
    for (int type = PL; type <= TR; type++)
    {
        ...
        {
            ns->Arc[type][direction] = new arrayindex_t *[Net::Card[type]];
    ...
    for (arrayindex_t i = 0; i < Net::Card[PL]; i++)
    {
        ...
        // pre-Zone
        ns->Arc[PL][PRE][i] = new arrayindex_t[Net::CardArcs[PL][PRE][i]];
        ...
        // post-Zone
        ns->Arc[PL][POST][i] = new arrayindex_t[Net::CardArcs[PL][POST][i]];
    ...
    for (arrayindex_t i = 0; i < Net::Card[TR]; i++)
    {
        // allocate memory for place's arcs (is copied later with transitions)
        // pre-Zone
        ns->Arc[TR][PRE][i] = new arrayindex_t[Net::CardArcs[TR][PRE][i]];
        ...
        // post-Zone
        ns->Arc[TR][POST][i] = new arrayindex_t[Net::CardArcs[TR][POST][i]];
    ...
    ns->CardDisabled = new arrayindex_t[Net::Card[PL] ]();
    ns->Disabled = new arrayindex_t *[Net::Card[PL]];
    for (arrayindex_t i = 0; i < Net::Card[PL]; i++)
    {
        ns->CardDisabled[i] = Place::CardDisabled[i];
        ns->Disabled[i] = new arrayindex_t[Net::CardArcs[PL][POST][i]];
        for (arrayindex_t j = 0; j < Net::CardArcs[PL][POST][i]; j++)
        {
...
NetState::NetState(const NetState &ons)
{
    ...
    // copy current scapegoats
    PositionScapegoat = new arrayindex_t[Net::Card[TR]];
    for (arrayindex_t i = 0; i < Net::Card[TR]; i++)
    {
    ...
    for (int type = PL; type <= TR; type++)
    {
        for (int direction = PRE; direction <= POST; direction++)
        {
            Arc[type][direction] = new arrayindex_t *[Net::Card[type]];
    ...
    for (arrayindex_t i = 0; i < Net::Card[PL]; i++)
    {
        // allocate memory for place's arcs (is copied later with transitions)
        // pre-Zone
        Arc[PL][PRE][i] = new arrayindex_t[Net::CardArcs[PL][PRE][i]];
        for (arrayindex_t j = 0; j < Net::CardArcs[PL][PRE][i]; j++)
        {
        ...
        // post-Zone
        Arc[PL][POST][i] = new arrayindex_t[Net::CardArcs[PL][POST][i]];
        for (arrayindex_t j = 0; j < Net::CardArcs[PL][POST][i]; j++)
        {
    ...
    for (arrayindex_t i = 0; i < Net::Card[TR]; i++)
    {
        // allocate memory for place's arcs (is copied later with transitions)
        // pre-Zone
        Arc[TR][PRE][i] = new arrayindex_t[Net::CardArcs[TR][PRE][i]];
        for (arrayindex_t j = 0; j < Net::CardArcs[TR][PRE][i]; j++)
        {
        ...
        // post-Zone
        Arc[TR][POST][i] = new arrayindex_t[Net::CardArcs[TR][POST][i]];
        for (arrayindex_t j = 0; j < Net::CardArcs[TR][POST][i]; j++)
        {
    ...
    // copy currently disabled
    CardDisabled = new arrayindex_t[Net::Card[PL] ]();
    Disabled = new arrayindex_t *[Net::Card[PL]];
    for (arrayindex_t i = 0; i < Net::Card[PL]; i++)
    {
        CardDisabled[i] = ons.CardDisabled[i];
        Disabled[i] = new arrayindex_t[Net::CardArcs[PL][POST][i]];
        for (arrayindex_t j = 0; j < Net::CardArcs[PL][POST][i]; j++)
        {
