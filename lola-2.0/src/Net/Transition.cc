...
first entry there.
*/
arrayindex_t *Transition::PositionScapegoat = NULL;
...
void Transition::checkEnabled(NetState &ns, arrayindex_t t)
{
    ...
    for (arrayindex_t i = 0; i < Net::CardArcs[TR][PRE][t]; ++i)
    {
        if (ns.Current[ns.Arc[TR][PRE][t][i]] < ns.Mult[TR][PRE][t][i])
        {
            ...
            const arrayindex_t scapegoat = ns.Arc[TR][PRE][t][i];
            if (ns.Enabled[t])
            {
            ...
            else
            {
                ...
                if (i > 0)
                {
                    ...
                    const arrayindex_t old_scapegoat = ns.Arc[TR][PRE][t][0];
                    arrayindex_t c = --ns.CardDisabled[old_scapegoat];
                    if (ns.PositionScapegoat[t] != c)
                    {
                        // transition not last in scapegoat's disabled list--> swap with last
                        const arrayindex_t other_t = ns.Disabled[old_scapegoat][c];
    ...
    if (!ns.Enabled[t])
    {
        ...
        ++ns.CardEnabled;
        const arrayindex_t old_scapegoat = ns.Arc[TR][PRE][t][0];
        const arrayindex_t c = --ns.CardDisabled[old_scapegoat];
        if (ns.PositionScapegoat[t] != c)
        {
...
{
    arrayindex_t *delta_value = new arrayindex_t[Net::Card[PL]]();
...
bool DEBUG__testEnabled(NetState &ns, arrayindex_t t)
{
    if (ns.Enabled[t])
    {
        for (arrayindex_t i = 0; i < Net::CardArcs[TR][PRE][t]; i++)
        {
            arrayindex_t p = ns.Arc[TR][PRE][t][i];
    ...
    {
        arrayindex_t p = ns.Arc[TR][PRE][t][0];
